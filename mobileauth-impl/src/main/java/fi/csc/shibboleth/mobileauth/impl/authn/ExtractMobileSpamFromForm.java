...
/**
 * An action that extracts a mobile number and possible spam prevention code from an HTTP
 * form, creates a {@link MobileUserContext}, and attaches it to the
...
public class ExtractMobileSpamFromForm extends AbstractExtractionAction {
	...
	protected void doExecute(@Nonnull final ProfileRequestContext profileRequestContext,
		...
		if (spamCode == null || spamCode.isEmpty()) {
			log.debug("{} No spamCode in the request, which is fine", getLogPrefix());

		} else if (StringUtils.isNotBlank(spamCode)) {
